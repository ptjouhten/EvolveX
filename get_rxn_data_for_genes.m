function [gene_data] = get_rxn_data_for_genes(gene_set,flux_data,model,bigger)
...
for i = 1:length(gene_set)
    ...
    for j = 1:length(rxn_ind_list)
        ...
        elseif bigger == 1 && flux_data(rxn_ind_list(j)) > gene_data(i)
            gene_data(i) = flux_data(rxn_ind_list(j));
        elseif bigger == 0 && flux_data(rxn_ind_list(j)) < gene_data(i)
            gene_data(i) = flux_data(rxn_ind_list(j));
        end
    end
end
