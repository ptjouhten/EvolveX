function [strength, coverage, ncomp, value, bc, sol, tarsol] = evolvex(components,inhibitors,model,flux_basis,flux_basis_dirs,inhibitor_rxn_matrix,reference_minmax,aerobic)
...
model.ub(components) = -1; %changed zero 04/02/21 pj
...
if isempty(sol.x)
   ...
   %fluxes as variables
    ...
    b(end,1) = -floor((model.c'*(sol.x))/tol)*tol; % added 1.1* 04/02/21 pj
